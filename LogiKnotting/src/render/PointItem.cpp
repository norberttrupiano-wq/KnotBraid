...
static constexpr qreal R = 3.0;

PointItem::PointItem(const QPointF& posMM)
    : m_pos(posMM)
{
    setPos(m_pos);
}
...
void PointItem::paint(QPainter* p,
                      const QStyleOptionGraphicsItem*,
                      QWidget*)
{
    p->setPen(Qt::NoPen);
    p->setBrush(QColor(0, 90, 255)); // bleu net
